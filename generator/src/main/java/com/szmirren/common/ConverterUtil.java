...
public class ConverterUtil {
	 ...
	 *          装换目标类
	 */
	public static void createDtoConfigToContent(DtoConfig dc, CreateDtoContent content) {
		if (!StrUtil.isNullOrEmpty(dc.getTableAlias())) {
			content.setTableAlias(dc.getTableAlias());
		}
		if (!StrUtil.isNullOrEmpty(dc.getPrimaryKey())) {
			content.setPrimaryKey(dc.getPrimaryKey());
		}
		if (dc.getTblPropertyValues() != null) {
			ArrayList<FieldAttribute> list = new ArrayList<>();
			ArrayList<FieldAttribute> cantNullAttrs = null;
			ArrayList<FieldAttribute> otherAttrs = null;

			for (TableAttributeDto c : dc.getTblPropertyValues()) {
				if (!c.getTdCreate()) {
					continue;
				}
				FieldAttribute attr = new FieldAttribute(c);
				...
				list.add(attr);
				// 设置主键的类型与主键的属性列
				if (dc.getPrimaryKey() != null && dc.getPrimaryKey().equals(c.getTdColumnName())) {
					content.setPrimaryKeyJdbcType(c.getTdJdbcType());
					content.setPrimaryKeyJavaType(c.getTdJavaType().getValue());
					content.setPrimaryKeyAttr(attr);
					continue;
