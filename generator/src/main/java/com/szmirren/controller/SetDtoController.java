...
import com.szmirren.models.TableAttributeDtoEditingCell;
import com.szmirren.options.DatabaseConfig;
import com.szmirren.options.DtoConfig;
 ...
 *
 */
public class SetDtoController extends BaseController {
	...
	// =======================控件区域===========================
	@FXML
	private TableView<TableAttributeDto> tblProperty;
	/** 是否生成 */
	@FXML
	private TableColumn<TableAttributeDto, Boolean> tdIsCreate;
	/** 列名 */
	@FXML
	private TableColumn<TableAttributeDto, String> tdColumn;
	/** jdbc数据类型 */
	@FXML
	private TableColumn<TableAttributeDto, String> tdSqlType;
	/** java数据类型 */
	@FXML
	private TableColumn<TableAttributeDto, ComboBox<String>> tdJavaType;
	/** java属性名 */
	@FXML
	private TableColumn<TableAttributeDto, String> tdField;
	 ...
	 */
	public void init() {
		LOG.debug("初始化SetCreateDtoAttribute...");
		LOG.debug("初始化SetCreateDtoAttribute->初始化属性...");
		// 添加右键删除属性
		...
		item.setOnAction(event -> {
			TableViewSelectionModel<TableAttributeDto> model = tblProperty.selectionModelProperty().get();
		...
		// 添加列
		Callback<TableColumn<TableAttributeDto, String>, TableCell<TableAttributeDto, String>> cellFactory = (
				TableColumn<TableAttributeDto, String> p) -> new TableAttributeDtoEditingCell();
		...
		tdColumn.setOnEditCommit((CellEditEvent<TableAttributeDto, String> t) -> {
			((TableAttributeDto) t.getTableView().getItems().get(t.getTablePosition().getRow())).setTdColumnName(t.getNewValue());
		});

		tdSqlType.setCellValueFactory(new PropertyValueFactory<>("tdJdbcType"));
		tdSqlType.setCellFactory(cellFactory);
		tdSqlType.setOnEditCommit((CellEditEvent<TableAttributeDto, String> t) -> {
			((TableAttributeDto) t.getTableView().getItems().get(t.getTablePosition().getRow())).setTdJdbcType(t.getNewValue());
		});
		...
		tdField.setOnEditCommit((CellEditEvent<TableAttributeDto, String> t) -> {
			((TableAttributeDto) t.getTableView().getItems().get(t.getTablePosition().getRow())).setTdField(t.getNewValue());
		});
		...
		if (indexController.getTemplateNameItems().contains(Constant.TEMPLATE_NAME_DTO)) {
			cboTemplate.setValue(Constant.TEMPLATE_NAME_DTO);
		}
		...
		if (indexController.getHistoryConfig() != null) {
			if (indexController.getHistoryConfig().getDtoConfig() == null) {
				loadConfig(getConfig());
			} else {
				LOG.debug("加载数据...从上一次保存获取");
				loadConfig(indexController.getHistoryConfig().getDtoConfig());
		...
		LOG.debug("初始化SetDtoAttribute-->成功!");
	}
	...
	public void initTablePropery(DatabaseConfig selectedDatabaseConfig, String selectedTableName) {
		try {
			LOG.debug("执行获取表的列数据...");
			List<TableAttributeDto> columns = DBUtil.getTableColumnsForCreateDto(selectedDatabaseConfig, selectedTableName);
	 ...
	 * @return
	 */
	public DtoConfig getConfig(String name) {
		LOG.debug("执行从数据库中获取配置文件...");
		try {
			DtoConfig config = ConfigUtil.getDtoConfig(name);
	 ...
	 * @return
	 */
	public DtoConfig getThisConfig() {
	 ...
	 * @return
	 */
	public DtoConfig getThisConfigHasProperty() {
	 ...
	 * @param config
	 */
	public void loadConfig(DtoConfig config) {
	 ...
	 * 加载表数据
	 */
	public void loadTablePropertyValues(List<TableAttributeDto> attr) {
		tblPropertyValues.clear();
		for (TableAttributeDto entity : attr) {
	...
	public void onSaveConfig(ActionEvent event) {
		...
		try {
			...
			ConfigUtil.saveDtoConfig(getThisConfig(), configName);
	...
	public void onAddPropertyToTable(ActionEvent event) {
		LOG.debug("执行添加自定义属性...");
		TableAttributeDto field = new TableAttributeDto(txtKey.getText(), txtValue.getText());
	...
	public void onCamel(ActionEvent event) {
		ArrayList<TableAttributeDto> list = new ArrayList<>(tblPropertyValues);
		tblPropertyValues.clear();
		if (chkFieldCamel.isSelected()) {
			for (TableAttributeDto entity : list) {
	...
	public void onCancel(ActionEvent event) {
		...
		if (result) {
			indexController.getHistoryConfig().setDtoConfig(null);
	...
	public void onConfirm(ActionEvent event) {
		indexController.getHistoryConfig().setDtoConfig(getThisConfigHasProperty());
		getDialogStage().close();
	}
