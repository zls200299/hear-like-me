...
@Setter
@ToString
public class MongoListenConfig {
    /** 设置的tableItem */
    private List<TableAttributeKeyValue> tableItem = new ArrayList<>();
    /** 生成模板的名字 */
    private String templateName = Constant.TEMPLATE_NAME_MONGO_LISTEN;
    /** 是否覆盖原文件 */
    private boolean overrideFile = false;
    /**
     * 初始化
     */
    public MongoListenConfig() {
        super();
    }
     ...
     *
     */
    public MongoListenConfig(ObservableList<TableAttributeKeyValue> item) {
        super();
     ...
     *
     */
    public MongoListenConfig(ObservableList<TableAttributeKeyValue> item, String templateName, boolean overrideFile) {
        super();
        if (item != null && !item.isEmpty()) {
            tableItem.addAll(item);
        }
