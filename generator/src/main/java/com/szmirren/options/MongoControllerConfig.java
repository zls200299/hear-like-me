...
@Setter
@ToString
public class MongoControllerConfig {
    /** 设置的tableItem */
    private List<TableAttributeKeyValue> tableItem = new ArrayList<>();
    /** 生成模板的名字 */
    private String templateName = Constant.TEMPLATE_NAME_MONGO_CONTROLLER;
    /** 是否覆盖原文件 */
    private boolean overrideFile = true;
    /**
     * 初始化
     */
    public MongoControllerConfig() {
        super();
    }
     ...
     *
     */
    public MongoControllerConfig(ObservableList<TableAttributeKeyValue> item) {
        super();
     ...
     *
     */
    public MongoControllerConfig(ObservableList<TableAttributeKeyValue> item, String templateName, boolean overrideFile) {
        super();
        if (item != null && !item.isEmpty()) {
            tableItem.addAll(item);
        }
