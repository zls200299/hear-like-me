...
import com.zhs.dao.ReadAloudAudioDao;
...
@Service
public class ReadAloudAudioServiceImpl extends ServiceImpl<ReadAloudAudioDao, ReadAloudAudio>
        implements ReadAloudAudioService {
    ...
    @Override
    @Transactional
    public R<ReadAloudAudio> addOrUpdate(ReadAloudAudioDto dto) {
        validate(dto);
        ensureCodeUnique(dto.getAudioCode(), dto.getId());

        ReadAloudAudio entity;
        boolean generationChanged;
        if (dto.getId() == null) {
            entity = new ReadAloudAudio();
    ...
    @Override
    public R<ReadAloudAudio> generate(Long id) {
        ReadAloudAudio entity = requireAudio(id);
    ...
    public R<String> deleteAudio(Long id) {
        ReadAloudAudio entity = requireAudio(id);
        Long referenceCount = readAloudItemDao.selectCount(new LambdaQueryWrapper<ReadAloudItem>()
                .eq(ReadAloudItem::getAudioBankId, id)
                .eq(ReadAloudItem::getIsDelete, 0));
        if (referenceCount != null && referenceCount > 0) {
            throw new ServiceException("该音频已被点读卡片引用，不能删除；可将其停用");
        }
    ...
    private void validate(ReadAloudAudioDto dto) {
        if (dto == null) {
        ...
        if (!ALLOWED_CHANNELS.contains(dto.getNChannels())) {
            throw new ServiceException("点读音频仅支持 2、4、8、16 通道");
        }
    ...
    private void validateEntity(ReadAloudAudio entity) {
        ReadAloudAudioDto dto = new ReadAloudAudioDto();
    ...
    private void ensureCodeUnique(String code, Long currentId) {
        LambdaQueryWrapper<ReadAloudAudio> wrapper = new LambdaQueryWrapper<ReadAloudAudio>()
                .eq(ReadAloudAudio::getAudioCode, code.trim());
        if (currentId != null) {
            wrapper.ne(ReadAloudAudio::getId, currentId);
        }
        if (count(wrapper) > 0) {
    ...
    private ReadAloudAudio requireAudio(Long id) {
        if (id == null) {
            throw new ServiceException("音频ID不能为空");
        }
        ReadAloudAudio entity = getOne(new LambdaQueryWrapper<ReadAloudAudio>()
    ...
    private boolean generationChanged(ReadAloudAudio old, ReadAloudAudioDto current) {
